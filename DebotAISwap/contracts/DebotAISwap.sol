...
contract DebotAISwap is Storage, IUniswapV3SwapCallback, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function takeFee(address tokenIn, uint256 amountIn) internal returns (uint256){
        ...
        if (( tokenIn == address(0) || tokenIn == WETH ) && address(this).balance > fee ) {
            ...
            require(success, "DebotAISwap: take fee error");
        } else
    ...
    function swapV2ExactIn(
        ...
        require(poolAddress != address(0), "DebotAISwap: invalid pool address");
        require(amountIn > 0, "DebotAISwap: amout in is zero");

        bool nativeIn = false;
        if (tokenIn == address(0)) {
            require(msg.value >= amountIn, "DebotAISwap: amount in and value mismatch");
            nativeIn = true;
            tokenIn = WETH;
            // refund
            ...
            if (amount > 0) {
              ...
              require(success, "DebotAISwap: refund ETH error");
            }
        }
        ...
        if (nativeOut) {
            ...
            require(success, "DebotAISwap: send ETH out error");
        } else {
            amountOut = IERC20Upgradeable(tokenOut).balanceOf(msg.sender).sub(balanceBefore);
        }
        require(
            amountOut >= amountOutMin,
            'DebotAISwap: insufficient output amount'
        );
    }
    ...
    function swapV2ExactOut(
        ...
        require(poolAddress != address(0), "DebotAISwap: invalid pool address");
        require(amountInMax > 0, "DebotAISwap: amout in max is zero");

        bool nativeIn = false;
        if (tokenIn == address(0)) {
            tokenIn = WETH;
            nativeIn = true;
            require(msg.value >= amountInMax, "DebotAISwap: amount in and value mismatch");
        }
        ...
        { // scope to avoid stack too deep errors
            ...
            require(amountIn + fee <= amountInMax, "DebotAISwap: excessive input amount");
            if(nativeIn) {
                pay(tokenIn, address(this), poolAddress, amountIn);
    ...
    function swapV2MultiHopExactOut(
        ...
        require(amountInMax > 0, "DebotAISwap: amount in max is zero");
        ...
        require(amounts[0] + fee <= amountInMax, 'DebotAISwap: excessive input amount');
    ...
    function swapV3ExactIn (
        ...
        require(params.amountIn > 0, "DebotAISwap: amount in is zero");

        if (params.tokenIn == address(0)) {
            params.tokenIn = WETH;
            require(msg.value >= params.amountIn, "DebotAISwap: amount in and value mismatch");
            // refund
        ...
        require(amountOut >= params.amountOutMinimum, "DebotAISwap: insufficient out amount");
    ...
    function swapV3ExactOut (
        ...
        require(params.amountInMaximum > 0, "DebotAISwap: amount in max is zero");

        bool nativeIn = false;
        if (params.tokenIn == address(0)) {
            nativeIn = true;
            params.tokenIn = WETH;
            require(msg.value >= params.amountInMaximum, "DebotAISwap: amount in max and value mismatch");
        }
        ...
        require(amountIn + fee <= params.amountInMaximum, "DebotAISwap: too much requested");
    ...
    function swapV3MultiHopExactIn (
        ...
        require(amountOut >= params.amountOutMinimum, 'DebotAISwap: too little received');
    }
    ...
    function swapMixedMultiHopExactIn (
        ...
        require(params.routes.length == 2, "DebotAISwap: only 2 routes supported");
    ...
    function swapMixedMultiHopExactOut(
        ...
        if (isStrEqual(params.routes[0], "v2") && isStrEqual(params.routes[1], "v2")) {
            ...
            require(amountIn2 == params.amountIn2, "DebotAISwap: not support fee-on-transfer token for V3");
